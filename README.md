...
- 相同序号、**相同内容**重传 → `200` 且 `"idempotent": true`（断网重传安全）。
  若该序号的确认行仍在、但分片正文因外部故障丢失或损坏（见下方
  `range_unavailable`），同内容重传会以原子写（临时文件 + fsync + rename）
  **自愈恢复正文**，随后区间预检立即恢复可用；
...
3. 对已确认序号的重传按内容判定：相同 → 幂等成功（若磁盘正文丢失或损坏，
   同内容重传原子恢复正文后仍返回幂等成功，会话可自愈）；不同 → 409 且不动旧数据；
